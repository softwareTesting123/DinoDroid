...
def addMenuBack(adbPath, easyoperate, doc_newresult, commands, port):
    global runable
    global scrollBol
    ...
    ##############menu
    menuTag=ENV.checkMenu(adbPath, commands, port)
    if menuTag:
        runableID=doc_newresult.createElement("runableID")
        #step.appendChild(runableID)
        '''
...
def translate(adbPath, root, root_new, doc_newresult, packageName, classNameStr, commands, port, loading):
    ...
    addMenuBack(adbPath, easyoperate,doc_newresult,commands, port)
...
def trans(adbPath, root,packageName,className, commands ,port):
    address="."
    doc_newresult=Document()
    root_new=doc_newresult.createElement("Result")
    
    loading=[0]#0 means false, 1 means true
    notEmpty=translate(adbPath, root, root_new, doc_newresult, packageName, className,commands ,port, loading)#translate from the xml from uiautomator.
